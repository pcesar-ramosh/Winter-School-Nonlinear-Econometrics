...
stop
stop
' No tocas de aquí en adelante por que son pronósticos se debe revisar el pronóstico
'====================================================='
'====================================================='
'====================================================='
...
' %%%%%%%%%%%%%%%%%%%%%%%
'__Holt Winters para pronóstico de variables __'
'%%%%%%%%%%%%%%%%%%%%%%%
pagecreate(page=hwinters) m 2005M01 2021M09
	' Seleccionamos la página donde se copiarán las variables
	pageselect hwinters
	wfdetails Name Display Update Type Description 
	' Copiamos las variables que requerimos
	for %y x20 x21 x23 x25 x26 x30 x31 x32  x41
		copy Variables\{%y}*
	next
	' Aumentamos la muestra para el pronóstico
	pagestruct(end=2021M12)
	' Método de Holt Winters
	for %w x20 x21 x23 x25 x26 x30 x31 x32  x41
		{%w}.smooth(m) {%w}_hwm 12
		'group series{%w} {%w} {%w}_hwm 
		'freeze(plot_{%w}) series{%w}.line 
	next

	for %w x20 x21 x23 x25 x26 x30 x31 x32  x41
' Pron´stico fuera la muestra de Octubre a Diciembre
		smpl 2021m10 2021m12
		series {%w} = {%w}_hwm
		smpl @all
	next
...
' --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### '
' ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- '
'    Modelos de umbrales discretos y modelos de transición suave logístico y exponencial
' --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### '
' ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- ### --- '

' ##########################'
' ##########################'
' Modelo regresión por umbrales discreto
' ##########################'
' ##########################'
pagecreate(page=mod_log_exp) m 2005M01 2021M12
	pageselect mod_log_exp
	' Copiamos las variables que requerimos
	for %k x20 x21 x23 x25 x26 x30 x31 x32  x41
		copy hwinters\{%k} *
	next
	' En tasas de variación anualizadas
	for %k x20 x21 x23 x25 x26 x30 x41
		series g_{%k} =  ({%k}/{%k}(-12)-1)*100
	next
...
' Estimación del modelo por regiones - Transición discreta
	equation tar_zzzzz.threshold(cov=hac, maxbreaks=2) g_x20 c g_x21 g_x25(-6) g_x26(-3) @nv x31 g_x20(-1) g_x21^2 x32(-7) @isperiod("2020m04") @thresh g_x20(-1)
	tar_zzzzz.fit(e) g_x20fz
	g_x20fz.displayname Regímenes var. créditos
	group series02 g_x20fz g_x20
...
' Con varias exógenas "plot_cred_z"
	...
	plot_cred_z.draw(shade, bottom, @rgb(255,255,217))  if g_x20>9.008718 and g_x20<12.63518
	'  Para datos mayores a 12.63518 que es la Región 3 ALTA
	plot_cred_z.draw(shade, bottom, @rgb(255,231,206))  if g_x20>12.63518
	' Graficamos dos lines horizontales  (Umbrales)
	plot_cred_z.draw(line, left, @rgb(0,0,0), pattern(7), linewidth(1))  12.63518
	plot_cred_z.draw(line, left, @rgb(0,0,0), pattern(7), linewidth(1))  9.008718
	plot_cred_z.addtext(0.05, 2.12, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,8,-b,-i,-u,-s))  "Threshold 1: 9"
	plot_cred_z.addtext(0.05, 1.58, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,8,-b,-i,-u,-s))  "Threshold 2: 12,3"	
	' Etiquetas para cada region, Regímenes  R-1, R-2 y R-3
	plot_cred_z.addtext(0.33,0.07, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(t) , font(Garamond,10,-b,-i,-u,-s))  "Regime 1"
	plot_cred_z.addtext(1.16,0.07, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,10,-b,-i,-u,-s))  "Regime 2"
	plot_cred_z.addtext(3.09,0.07, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,10,-b,-i,-u,-s))  "Regime 3"
	plot_cred_z.addtext(4.66,0.07, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,10,-b,-i,-u,-s))  "Regime 2"
	plot_cred_z.addtext(5.38,0.07, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,10,-b,-i,-u,-s))  "Regime 1"	
	' Pornóstico dentro y fuera la muestra dentro 2005m01 - 2021m06  fuera la muestra 2021m07 - 2021 m12
	plot_cred_z.draw(line, bottom, @rgb(0,0,160), pattern(1), linewidth(1), top) 2021M09
	plot_cred_z.addtext(5.53,-0.19, textcolor(@rgb(0,0,160)), fillcolor(@rgb(0,0,160)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,10,-b,-i,-u,-s))  "Sep 2021"
	' cuadro 
	'plot_cred_z.addrect pos(1,1) width(2) height(0.01) angle(0)  linewidth(0.5)
	' Promedio para la tasa de var.% de los créditos Regimje 1
	smpl @all if g_x20<9.008718 
	scalar R1 = @mean(g_x20) 
	smpl @all
	' Promedio para la tasa de var.% de los créditos Regimje 2
	smpl @all if g_x20>9.008718 and g_x20<12.63518
	scalar R2 = @mean(g_x20) 
	smpl @all
	' Promedio para la tasa de var.% de los créditos Regimje 3
	smpl @all if g_x20>12.63518
	scalar R3 = @mean(g_x20) 
	smpl @all
	plot_cred_z.addtext(0.49,0.26, textcolor(@rgb(0,0,160)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,10,-b,-i,-u,-s))  "5.43"
	plot_cred_z.addtext(1.25,0.26, textcolor(@rgb(0,0,160)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,10,-b,-i,-u,-s))  "10.82"
	plot_cred_z.addtext(3.20,0.26, textcolor(@rgb(0,0,160)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,10,-b,-i,-u,-s))  "18.79"
	plot_cred_z.addtext(4.79,0.26, textcolor(@rgb(0,0,160)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,10,-b,-i,-u,-s))  "10.82"
	plot_cred_z.addtext(5.50,0.26, textcolor(@rgb(0,0,160)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(0,0,0)), just(l) , font(Garamond,10,-b,-i,-u,-s))  "5.43"
...
' Brecha del producto
	x26.x12
	x26_sa.hpf pib_trend
	x26_sa.hpf(lambda=200) pib_trendi
	series brecha = ((x26_sa-pib_trend)/pib_trend)*100
	series brechas = ((x26_sa-pib_trendi)/pib_trendi)*100
	brecha.displayname Brecha del PIB
	brechas.displayname Output gap
...
'___Modelo de transición suave Logístico___'
'%%%%%%%%%%%%%%%%%%%%%%
'**** este el mejor modelo ****
' Modelo exponencial (Umbral - Brecha del PIB)
	smpl 2006m01 @last
	equation stlmo.threshold(type=smooth, optmethod=newton, maxit=100000, cov=white) g_x20 g_x23 g_x25 x31 c x32(-3) @nv g_x30 g_x26 g_x21(-6) g_x21^2 @thresh brecha
	freeze(logistic_stlmo) stlmo.strwgts
	stlmo.fit(e) g_x20_stlmo
	group series_log g_x20 brecha 
'	freeze(plot_stlmo) g_x20.line
		smpl 2006m01 2021m12
	freeze(plot_stlmo)  series_log.line
' Gráfico del modelo logístico "plot_stlmo"
	' Ticks para el eje izquierdo
	plot_stlmo.axis(l) mintickcount(5)
	' Sombras en cada área - para las regiones pero respecto al output_gap menores a 2.369022
	plot_stlmo.draw(shade, bottom, @rgb(216,222,222))  if brecha<1.76
	plot_stlmo.addtext(2.4,2.76, textcolor(@rgb(0,0,0)), fillcolor(@rgb(231,235,235)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Debajo del umbral"
	' Sombras en cada área - para las regiones pero respecto al output_gap menores a 2.369022
	plot_stlmo.draw(shade, bottom, @rgb(255,255,217))  if brecha>1.76
	plot_stlmo.addtext(4.67,2.76, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,217)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Encima del umbral"
	' Umbral gamma 1.76
	plot_stlmo.draw(line, left, @rgb(255,128,0), pattern(1), linewidth(1)) 1.76 
	plot_stlmo.addtext(2.4,1.71, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(255,128,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Threshold = 1.76"
	' Promedio por debajo del umbral repsecto al output_gap
	smpl @all if brecha<1.76 
	scalar R_1_debajo = @mean(g_x20) 
	smpl @all
	' Promedio por encima del umbral repsecto al output_gap
	smpl @all if brecha>1.76
	scalar R_2_encima = @mean(g_x20) 
	smpl @all
	' las tasas óptima de cada regimen
	plot_stlmo.addtext(2.28,0.05, textcolor(@rgb(0,0,0)), fillcolor(@rgb(231,235,235)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Tasa var. cred. 14.46"
	plot_stlmo.addtext(4.78,0.05, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,217)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Tasa var. cred. 7.77"
...
'%%%%%%%%%%%%%%%%%%%%%%
'_Modelo de transición suave Exponencial__'
'%%%%%%%%%%%%%%%%%%%%%%

' Modelo exponencial (Umbral - Brecha del PIB)
	smpl 2006m01 @last
	equation stemo.threshold(type=smooth, optmethod=newton, maxit=100000, cov=white, smoothtrans=exponential) g_x20 g_x23 c x32 @nv g_x30 g_x26 g_x25 x31 @thresh brecha
	freeze(exponential_stemo) stemo.strwgts
	stlmo.fit(e) g_x20_stemo
	group series_exp g_x20 brecha 
'	freeze(plot_stlmo) g_x20.line
	freeze(plot_stemo)  series_log.line

' Gráfico del modelo exponencial "plot_stemo"
	' Ticks para el eje izquierdo
	plot_stemo.axis(l) mintickcount(5)
	' Sombras en cada área - para las regiones pero respecto al output_gap menores a 4.744762
	plot_stemo.draw(shade, bottom, @rgb(216,222,222))  if brecha<4.744762
	plot_stemo.addtext(2.4,2.76, textcolor(@rgb(0,0,0)), fillcolor(@rgb(231,235,235)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Debajo del umbral"
	' Sombras en cada área - para las regiones pero respecto al output_gap menores a 4.744762
	plot_stemo.draw(shade, bottom, @rgb(255,255,217))  if brecha>4.744762
	plot_stemo.addtext(4.67,2.76, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,217)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Encima del umbral"
	' Umbral gamma 2.369022
	plot_stemo.draw(line, left, @rgb(255,128,0), pattern(1), linewidth(1)) 4.744762
	plot_stemo.addtext(2.4,1.71, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(255,128,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Threshold = 4.74"
	' Promedio por debajo del umbral repsecto al output_gap
	smpl @all if brecha<4.744762
	scalar R_11_debajo = @mean(g_x20) 
	smpl @all
	' Promedio por encima del umbral repsecto al output_gap
	smpl @all if brecha>4.744762
	scalar R_22_encima = @mean(g_x20) 
	smpl @all
	' las tasas óptima de cada regimen
	' Por debajo
	plot_stemo.addtext(2.28,0.05, textcolor(@rgb(0,0,0)), fillcolor(@rgb(231,235,235)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Tasa var. cred. 13.54"
	' Por encima
	plot_stemo.addtext(4.78,0.05, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,217)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Tasa var. cred. 8.8"

' Modelo exponencial (Umbral - Tasa de crecimiento del PIB)
	equation ste_02.threshold(type=smooth, optmethod=newton, maxit=100000, cov=white, smoothtrans=exponential) g_x20 g_x23 c x32 @nv g_x30 g_x26 g_x25 x31 @thresh g_x26

' Modelo exponencial (Umbral - Cartera en Mora)	  
	equation stemo01.threshold(type=smooth, optmethod=newton, maxit=100000, cov=white, smoothtrans=exponential) g_x20 c g_x23 x32 x31 @nv g_x30 g_x25 g_x26 g_x25 @thresh g_x21
	freeze(exponential_stemo01) stemo01.strwgts
	'stlmo.fit(e) g_x20_stemo01
	group series_exp01 g_x20 g_x21 
	freeze(plot_stemo01) series_exp01.line
	'freeze(plot_stemo01)  g_x20.line

' Gráfico del modelo exponencial "plot_stemo01"
	plot_stemo01.legend columns(2)
	' La varibale 1 que se vaya al eje secundario
	plot_stemo01.setelem(1) axis(r)
	' Ticks para el eje izquierdo
	plot_stemo01.axis(l) mintickcount(5)
	' Sombras en cada área - para las regiones pero respecto al MORA menores a -4.709091
	plot_stemo01.draw(shade, bottom, @rgb(216,222,222))  if g_x21<-4.709091
	plot_stemo01.addtext(1.07,2.76, textcolor(@rgb(0,0,0)), fillcolor(@rgb(231,235,235)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Debajo del umbral"
	' Sombras en cada área - para las regiones pero respecto al output_gap menores a 4.744762
	plot_stemo01.draw(shade, bottom, @rgb(255,255,217))  if g_x21>-4.709091
	plot_stemo01.addtext(3.56,2.76, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,217)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Encima del umbral"
	' Umbral gamma -4.709091
	plot_stemo01.draw(line, left, @rgb(255,128,0), pattern(1), linewidth(1)) -4.709091
	plot_stemo01.addtext(2.4,1.71, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(255,128,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Threshold = -4.71"
	' Promedio por debajo del umbral repsecto al Mora
	smpl @all if g_x21<-4.709091
	scalar R_111_debajo = @mean(g_x20) 
	smpl @all
	' Promedio por encima del umbral repsecto al Mora
	smpl @all if g_x21>-4.709091
	scalar R_222_encima = @mean(g_x20) 
	smpl @all
	' las tasas óptima de cada regimen
	' Por debajo
	plot_stemo01.addtext(2.28,0.05, textcolor(@rgb(0,0,0)), fillcolor(@rgb(231,235,235)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Tasa var. cred. 11.62"
	' Por encima
	plot_stemo01.addtext(4.78,0.05, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,217)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Tasa var. cred. 14.8"



	' Modelo exponencial (Umbral - Tasa de crecimiento del PIB)
	equation ste_03.threshold(type=smooth, optmethod=newton, maxit=100000, cov=white, smoothtrans=exponential) g_x20 g_x23 c x32 @nv g_x30 g_x26 g_x25 x31 @thresh g_x26

	group seriespib g_x20 g_x26
smpl 2006m01 2021m12
	freeze(plot_ste03)  seriespib.line
	'freeze(plot_ste03)  g_x20.line

' Gráfico del modelo exponencial "plot_ste03"
	plot_ste03.legend columns(2)
	' La varibale 1 que se vaya al eje secundario
	plot_ste03.setelem(1) axis(r)
	' Ticks para el eje izquierdo
	plot_ste03.axis(l) mintickcount(5)
	' Sombras en cada área - para las regiones pero respecto al MORA menores a 4.870667
	plot_ste03.draw(shade, bottom, @rgb(216,222,222))  if g_x26<4.870667
	plot_ste03.addtext(1.07,2.76, textcolor(@rgb(0,0,0)), fillcolor(@rgb(231,235,235)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Debajo del umbral"
	' Sombras en cada área - para las regiones pero respecto al output_gap menores a 4.870667
	plot_ste03.draw(shade, bottom, @rgb(255,255,217))  if g_x26>4.870667
	plot_ste03.addtext(3.56,2.76, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,217)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Encima del umbral"
	' Umbral gamma 4.487332
	plot_ste03.draw(line, left, @rgb(255,128,0), pattern(1), linewidth(1)) 4.870667
	plot_ste03.addtext(2.4,1.71, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,255)), framecolor(@rgb(255,128,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Threshold = 4.87"
	' Promedio por debajo del umbral repsecto al Mora
	smpl @all if g_x26<4.870667
	scalar R_cre1_debajo = @mean(g_x20) 
	smpl @all
	' Promedio por encima del umbral repsecto al Mora
	smpl @all if g_x26>4.870667
	scalar R_cre2_encima = @mean(g_x20) 
	smpl @all
	' las tasas óptima de cada regimen
	' Por debajo
	plot_ste03.addtext(2.28,0.05, textcolor(@rgb(0,0,0)), fillcolor(@rgb(231,235,235)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Tasa var. cred. 12.22"
	' Por encima
	plot_ste03.addtext(4.78,0.05, textcolor(@rgb(0,0,0)), fillcolor(@rgb(255,255,217)), framecolor(@rgb(0,0,0)), sb, just(l) , font(Garamond,10,-b,-i,-u,-s))  "Tasa var. cred. 14.83"
	' Para guardar el workfile del modelo
wfsave(c) "variables_tip_credito_sector1.wf1"
stop
stop
stop
stop
...
' Para el gráfico logístico
	...
	equation stl_06.threshold(type=smooth, optmethod=newton, maxit=100000, cov=white) g_x20 g_x23 g_x25 x31 c x32(-3) @nv g_x30 g_x26 g_x21(-6) g_x21^2 @thresh brecha
	equation stl_07.threshold(type=smooth, optmethod=newton, maxit=100000, cov=white) g_x20 g_x23 g_x25 x31 c x32(-3) @nv g_x30 g_x26 g_x21(-6) g_x21^2 @isperiod("2021m04") @isperiod("2020m05") @isperiod("2019m10") @thresh g_x26
	freeze(Logistico) stl_05.strwgts
	stl_05.displayname logisitic regression
...
' %%%%%%%%%%%%%%%%%%%%%%%
'__Modelo VAR para pronóstico de variables __'
'%%%%%%%%%%%%%%%%%%%%%%%
pagecreate(page=VAR) m 2005M01 2021M06
	' Seleccionamos la página donde se copiarán las variables
	pageselect VAR
	wfdetails Name Display Update Type Description 
	' Copiamos las variables que requerimos
	for %y x20 x21 x23 x25 x26 x30 x31 x32  x41
		copy Variables\{%y}*
	next
' Especifiación de los rezagos del VAR
	var var01.ls 1 2 dlog(x20) dlog(x21) dlog(x23) dlog(x25) dlog(x26) dlog(x30) x31 x32 dlog(x41)

' Test de exclusión de rezagos
	freeze(exc_lags) var01.laglen(8)
'Criterio de AIC y  FPE: Final prediction error	sugieren 2 rezagos	
	var var02.ls 1 1 dlog(x20) dlog(x21) dlog(x23) dlog(x25) dlog(x26) dlog(x30) x31 x32 dlog(x41)
' Criterio de BIC y HQC sugieren 1 rezago
...
'====================================================='
'====================================================='
	var var03.ls 1 1 dlog(x20) dlog(x21) dlog(x23) dlog(x25) dlog(x26) dlog(x30) x31 x32 dlog(x41)
' Raíces del polinomio característico Círculo Unitartio
	freeze(raices_pol) var03.arroots(graph)
